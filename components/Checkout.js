...
export default function Checkout() {
  ...
  try {
    ...
    if (data.IsSuccess && data.Data && data.Data.InvoiceURL) {
      ...
      const errorMessage = data.Message || data.message || 'Failed to create payment session';
      setError('MyFatoorah error: ' + errorMessage);
      
      // Log validation errors if they exist
      if (data.ValidationErrors && data.ValidationErrors.length > 0) {
        console.error('Validation Errors:', data.ValidationErrors);
        const validationMessages = data.ValidationErrors.map(err => err.Error).join(', ');
        setError(`MyFatoorah validation error: ${validationMessages}`);
      }
    }
  } catch (err) {
    setError('MyFatoorah network error: ' + err.message);
  ...
  const renderPaymentMethodSelector = () => (
    <div className="payment-method-selector" style={{ marginBottom: '30px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '15px' }}>
        ...
        {/* MyFatoorah */}
        <div 
          className={`payment-option ${selectedPaymentMethod === PAYMENT_METHODS.MYFATOORAH ? 'selected' : ''}`}
          style={{
            border: selectedPaymentMethod === PAYMENT_METHODS.MYFATOORAH ? '2px solid #007bff' : '2px solid #ddd',
            borderRadius: '8px',
            padding: '15px',
            cursor: 'pointer',
            textAlign: 'center',
            backgroundColor: selectedPaymentMethod === PAYMENT_METHODS.MYFATOORAH ? '#f8f9fa' : '#fff'
          }}
          onClick={() => handlePaymentMethodChange(PAYMENT_METHODS.MYFATOORAH)}
        >
          <div style={{ fontSize: '24px', marginBottom: '8px' }}>🧾</div>
          <div style={{ fontWeight: 'bold', marginBottom: '5px' }}>MyFatoorah</div>
          <div style={{ fontSize: '12px', color: '#666' }}>Card, KNET, STC Pay</div>
        </div>
  ...
  const renderCustomerForm = () => {
    // Show customer form for both PayTabs and MyFatoorah
    if (selectedPaymentMethod !== PAYMENT_METHODS.PAYTABS && selectedPaymentMethod !== PAYMENT_METHODS.MYFATOORAH) {
      return null;
    }
  ...
  const renderPaymentInterface = () => {
    switch (selectedPaymentMethod) {
      ...
      case PAYMENT_METHODS.MYFATOORAH:
        return (
          <div style={{ marginTop: '20px' }}>
            <button
            ...
            >
              {loading ? 'Processing...' : `Pay ${orderData.currency} ${orderData.amount} with MyFatoorah`}
            </button>
          </div>
        );
  ...
  return (
    <div style={{ 
      ...
      {/* Customer Form (for PayTabs and MyFatoorah) */}
      {renderCustomerForm()}
